		...
		if (imcPM.matches()) {
            ...
            if (outDir==null){
                ...
                if (!imcPrefixM.matches()){
                    ...
                    //System.out.println("----WATER----------> model file name: "+layerModelFileName);
                    //System.out.println("----WATER----------> prefixRegex: "+prefixRegex);
                    //System.out.println("----WATER----------> ERROR: No match found");
...
//                outDir=new File(layerModelFile.getParentFile(),outDirComposedName);
    ...
    while (i<imagesListSize){
		...
		File newCommandFile;
        try {
            final File mergedImage=util.mergeWaterImage(outDir,args.get(0),args.get(1),outFileBaseName);
